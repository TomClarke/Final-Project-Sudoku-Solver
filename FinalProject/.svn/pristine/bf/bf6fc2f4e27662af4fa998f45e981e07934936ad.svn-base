...
public class SavedPuzzles {
	...
	for (int i = 0;i < 9; i++ ){
		for (int j = 0;j < 9; j++ ){
		board.sudokuBoard[i][j] = results[(j*9) + i] ;
		}
	}
	...
	public SudokuBoard save(String[] output) throws ParserConfigurationException{
		...
		board = new SudokuBoard();
		board.date = date.toString();
		int count = 0;
		for (int i = 0;i < 9; i++ ){
			for (int j = 0;j < 9; j++ ){
				items.add(output[count]);
			board.sudokuBoard[j][i] =Integer.parseInt(items.get(count));
			count++;
			}
	...
	public SudokuBoard load(String date) throws XPathExpressionException, ParserConfigurationException, SAXException, IOException{
